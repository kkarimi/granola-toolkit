...
| P1       | In Progress | M    |              | Gran / KB Plugin Seam | Move markdown vault and Obsidian-facing publish behavior behind Yazd knowledge-base plugin interfaces.      | Publishing should become source-agnostic.                         |
| P1       | Pending     | M    |              | SDK / Real Boundaries | Make the SDK depend on real external packages instead of re-exporting root source directly.                 | The SDK is not a real package boundary yet.                       |
