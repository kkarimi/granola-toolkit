...
| P1       | Pending | L    |              | Web / Browser Controller Hooks           | Split the Solid app runtime into narrower hooks or controllers for server/auth, browse, review, and harness flows.                      | `App.tsx` still acts like a giant browser runtime and makes every page flow touch one file.     |
| P2       | Pending | M    |              | Server / HTTP Route Modules              | Break the local HTTP server into route modules for auth, meetings, automation, exports, and web assets.                                 | `startGranolaServer()` is still a full router in one function and hard to extend safely.        |
