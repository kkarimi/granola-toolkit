...
| P1       | In Progress | M    |              | PKM / Target Contract  | Define the PKM target plugin contract for filesystem vaults, API targets, review mode, and idempotent publish identity.         | We need clean target boundaries before adding more destinations.    |
| P1       | Pending     | L    |              | PKM / Markdown Vault   | Build a first-class generic markdown vault target with stable paths, linked artifacts, and rerun-safe publishing.               | This gives the broadest immediate value across PKM tools.           |
