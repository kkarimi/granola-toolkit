...
`granola web` either reuses the running background service or starts a fresh local browser runtime
at `/`, then opens it in your default browser unless you pass `--open=false`.
...
If a background service is already running and you do not pass runtime overrides like `--port`,
`--hostname`, `--network`, `--password`, or `--sync-interval`, `granola web` will open that
