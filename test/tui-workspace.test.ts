...
function createWorkspaceHarness(
  ...
  const app: GranolaTuiApp = {
    ...
    inspectYazdSource: vi.fn(async () => ({
      id: "gran",
      label: "Gran",
      product: "gran" as const,
    })),
    inspectSync: vi.fn(async () => state.sync),
    listYazdSourceItems: vi.fn(async () => ({ items: [], source: "index" as const })),
    fetchYazdSourceItem: vi.fn(),
    buildYazdSourceArtifacts: vi.fn(),
    listYazdSourceChanges: vi.fn(async () => ({ changes: [] })),
